...
def download_file(session, service, file_spec, stats, chunk_size=128, num_attempts=10, **kwargs):
    ...
    for attempts_left in reversed(range(num_attempts)):
        ...
        try:
            ...
            with stats['lock']:
                data_size = os.path.getsize(tmp_path)
...
def download_files(file_specs, service, num_threads=32, status_delay=0.2, timing_window=50, **download_kwargs):
    ...
    for spec in missing_specs:
        spec_queue.put(spec)
    thread_kwargs = dict(spec_queue=spec_queue, service=service, exception_queue=exception_queue, stats=stats, download_kwargs=download_kwargs)
...
def _download_thread(spec_queue, service, exception_queue, stats, download_kwargs):
    with requests.Session() as session:
        while not spec_queue.empty():
            spec = spec_queue.get()
            try:
                download_file(session, service, spec, stats, **download_kwargs)
            except:
...
def run(tasks, **download_kwargs):
    ...
    if not os.path.isfile(json_spec['file_path']) or not os.path.isfile('LICENSE.txt'):
        ...
        download_files([json_spec, license_specs['json']], service, **download_kwargs)
    ...
    if len(specs):
        ...
        download_files(specs, service, **download_kwargs)
